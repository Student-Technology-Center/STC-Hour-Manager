<?php
/*
  archivedTicketList.php

  Author: Bryan Holdt
  Created: October 30, 2014

  This page is responsible for displaying all the archived tickets
  that live in the HARDWARE_TICKET_ARCHIVE table. This page also works
  very similar to the index page.
*/
      ...
      <!--<div><h4>Up for grabs:</h4></div>-->
			<table id="hourTable" class="table table-striped text-center">
      		 <thead>
                ...
                <th class="text-center">Fixed By</th>
       			</tr>
      		 </thead>
      		 <tbody>
       	   <?php
              ...
              //$curDate = date('Y-m-d');
              $result = mysql_query("SELECT * FROM HARDWARE_TICKET_ARCHIVE ORDER BY date_reported desc", $link) or die(mysql_error());
              $num_rows = mysql_num_rows($result);

              echo "<script type='text/javascript'> window.onload=load; </script>";
              if($num_rows == 0){
                echo "<b>" . "No archived hardware tickets yet." . "</b";
              }
